        ...
        for (i = 0; i < req_num; i++){
            ...
            set_callback(req[i], &crypto_complete);
                // Set value for ctx (testcase_1)
                    // INFO
            req[i]->crypto_dsc.info = testcase_2_in.crypto_dsc.info;
                    // ICV
            memcpy(req[i]->crypto_dsc.icv, testcase_2_in.crypto_dsc.icv, ICV_SIZE); 
                    // KEY
            memcpy(req[i]->crypto_dsc.key, testcase_2_in.crypto_dsc.key, KEY_SIZE); 
                    // IV
            req[i]->crypto_dsc.iv = testcase_2_in.crypto_dsc.iv;
                    // AAD
            memcpy(req[i]->crypto_dsc.aad, testcase_2_in.crypto_dsc.aad, AAD_SIZE); 
