        ...
        for (i = 0; i < req_num; i++){
            ...
            set_callback(req[i], &crypto_complete);
                // Set value for ctx (testcase_1)
                    // INFO
            req[i]->crypto_dsc.info = testcase_7_in.crypto_dsc.info;
                    // ICV
            memcpy(req[i]->crypto_dsc.icv, testcase_7_in.crypto_dsc.icv, ICV_SIZE); 
                    // KEY
            memcpy(req[i]->crypto_dsc.key, testcase_7_in.crypto_dsc.key, KEY_SIZE); 
                    // IV
            req[i]->crypto_dsc.iv = testcase_7_in.crypto_dsc.iv;
                    // AAD
            memcpy(req[i]->crypto_dsc.aad, testcase_7_in.crypto_dsc.aad, AAD_SIZE); 

            set_ctx(req[i], ctx);

            // Set outbound info -- testcase 1
            set_tag(req[i], 16, 0x50, (u32 *)kmalloc(16, GFP_ATOMIC | GFP_KERNEL));
        }
        // Set value for buffer - skip if you had buffer
        for (i = 0; i <  req_num; i ++){
            for (j =0; j < TESTCASE_1_LENGTH/4; j++){
                buff[i][j] = testcase_7_in.data[j];
            }
        }
